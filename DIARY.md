...
### 16–17 июня 2026 ⭐
- **Сессия: код-ревью, ребрендинг «Сонм», устойчивость сети, тесты**
- Полное ревью изменений — всё сверено с боевой базой через MCP (триггер профиля, сигнатуры RPC, CHECK-ограничения, типы полей совпадают с кодом)
- **Ребрендинг «Сонм»:** палитра дизайн-системы (Color.kt/Theme.kt — Crimson/Teal/Purple/Amber), вариативные шрифты Unbounded + Manrope (Type.kt), логотип на входе/регистрации, адаптивная иконка, splash, app_name → «Сонм»
- **Регистрация переведена на серверный триггер** `handle_new_user` (читает username из метаданных) — убран ручной INSERT профиля из клиента; проверено в бою: профиль создаётся
- Тема снова следует системной (убрали форс светлой — иначе белые иконки статус-бара на светлом фоне в тёмном режиме)
- Маппинг ошибок auth вынесен в чистую функцию `mapAuthError` (тестируемо)
- **Юнит-тесты (18 шт.):** сериализаторы Event (status/payment + fallback), `mapAuthError`, `retryingRead`
- **БД-миграция:** RLS-политики переведены на `(select auth.uid())` (быстрее на объёме), индексы под джойны `get_feed` по тегам
- **Найдена и устранена причина «бесконечных failed» в ленте:** OkHttp переиспользовал «мёртвые» keep-alive соединения (роутер/сеть тихо роняют half-open). Фикс в SupabaseClient: `pingInterval(5s)` + `retryOnConnectionFailure` + короткий пул. Сервер был ни при чём (`get_feed` = 40 мс)
- Стратегия повтора чтений вынесена в переиспользуемый `retryingRead` (core/network) — записи намеренно не оборачиваем (идемпотентность)
- Безопасное чтение local.properties + fallback на env, `allowBackup=false`, таймауты OkHttp 15→10с
- Проверено на телефоне (Samsung S10e): 8 обновлений за ~70с с простоями — 0 провалов
...
| Лента уходила в «бесконечные failed» после нескольких обновлений | OkHttp переиспользовал мёртвое keep-alive соединение — pingInterval + retryOnConnectionFailure в SupabaseClient | 17 июн |
| Долгая крутилка (до ~47с) при сетевом сбое ленты | Короткий таймаут на попытку (5с) + 1 повтор вместо 3×15с | 17 июн |
| signUp создавал профиль вручную (хрупко, можно остаться без профиля) | Серверный триггер handle_new_user из метаданных — проверено в бою | 16 июн |
| Белые иконки статус-бара на светлом фоне в тёмной теме телефона | Убран форс светлой темы — следуем системной | 16 июн |
...
| Профиль через серверный триггер, не ручной INSERT | Атомарно и надёжно — нельзя оказаться авторизованным без профиля | 16 июн |
| pingInterval(5s) на движке OkHttp | Мобильная сеть тихо роняет keep-alive соединения; HTTP/2-пинги детектят мёртвые | 17 июн |
| Ретрай только для чтений (retryingRead), не для записей | Повтор записи при потерянном ответе = дубль / двойное списание | 17 июн |
| Тема следует системной (тёмную доделаем позже) | Форс светлой ломал согласованность иконок статус-бара | 16 июн |
...
*Создан: 29 марта 2026 | Обновлён: 17 июня 2026*
